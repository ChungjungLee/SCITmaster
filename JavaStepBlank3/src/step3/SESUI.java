		...
		while(true) {
			...
			switch(no){
				...
				case 2: 
					...
					Human found = manager.findHuman(getJuminFromUser());
					if (found == null) {
				...
				case 3: 
					...
					boolean isDeleted = manager.deleteHuman(getJuminFromUser());
					if (isDeleted) {
		...
		while(true){
			...
			switch (no) {
				...
				case 3:
					...
					boolean isAdded = manager.insertHuman(h);
					//정상등록되었는지 동일한 주민번호가 존재하는지 출력한다.
					if (isAdded) {
						System.out.println("[정상 등록되었습니다.]");
					} else {
						System.out.println("[이미 존재하는 주민번호입니다.]");
					}
